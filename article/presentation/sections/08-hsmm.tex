...
\begin{frame}{Baseline 3: HSMM \small(Cont 2010, эвристика)}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \[
      \text{stay} \;/\; \text{advance} \;/\; \text{skip} \;/\;
      \underbrace{\text{leap}}_{\text{пропуск 2 состояний}}
    \]

    \column{0.45\textwidth}
    \centering
    \input{figures/tikz/hsmm-duration.tex}
    \figcaption{HMM $+$ распределение длительности $p_i(\tau)$ над каждым
             state. Переходы зависят от elapsed в state.}
  \end{columns}

  \vspace{0.2em}
  {\footnotesize \textbf{Добавляет к HMM:} учёт длительностей
   $\Rightarrow$ правильное поведение при rubato и лишних нотах.\quad
