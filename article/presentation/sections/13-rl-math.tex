...
\begin{frame}{Математика RL-слоя: состояние, действие, награда}
  ...
  длины пьесы}):
  \[
    s_t = \bigl(\,\underbrace{\hat\alpha_t}_{7},\
                  \underbrace{\tau_{t-K:t}}_{K=5},\
                  \underbrace{e_{t-K:t}}_{K=5},\
                  \underbrace{\hat\varphi(t)/N}_{1}\,\bigr)
    \;\Rightarrow\; 7+5+5+1 = \mathbf{18}
  \]
  \begin{itemize}\setlength{\itemsep}{0.15em}
    \item $\hat\alpha_t$ --- сжатка HSMM-распределения по $N$ позициям
          в \textbf{7} скаляров: $\max$, энтропия, $\arg\max/N$,
          top-3 mass, 3 индекса top-3;
    \item $\tau_{t-K:t}$ --- последние $K{=}5$ оценок темпа;\quad
          $e_{t-K:t}$ --- $K{=}5$ эмиссионных ошибок HSMM;
